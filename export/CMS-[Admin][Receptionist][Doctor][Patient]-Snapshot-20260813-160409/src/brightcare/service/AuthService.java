...
public class AuthService {
    ...
    public static class Sha256PasswordHasher implements PasswordHasher {
        ...
        public boolean matches(String plainTextPassword, String storedPasswordHash) {
            ...
            String[] storedHashes = storedPasswordHash.split("\\|");
            for (int i = 0; i < storedHashes.length; i++) {
                String storedValue = storedHashes[i].trim();
                if (plainTextPassword.equals(storedValue) || hashedPassword.equalsIgnoreCase(storedValue)) {
                    return true;
                }
            }
            return false;
