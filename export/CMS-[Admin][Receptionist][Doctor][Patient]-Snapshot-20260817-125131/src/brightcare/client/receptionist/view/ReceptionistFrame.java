...
public class ReceptionistFrame extends javax.swing.JFrame {
    ...
    private JComboBox<PatientChoice> createAppointmentPatientBox;
    private JComboBox<DoctorChoice> createAppointmentDoctorBox;
    private JTextField createAppointmentDateField;
    private JTextField createAppointmentTimeField;
    private JCheckBox createAppointmentActiveBox;
    private JTextField createAppointmentReasonField;
    ...
    private JPanel createAppointmentPanel() {
        ...
        tabs.addTab("Create Appointment", createAppointmentCreateTab());
        tabs.addTab("Update / Cancel", createAppointmentUpdateTab());
        panel.add(tabs, BorderLayout.CENTER);
        return panel;
    }

    private JPanel createAppointmentCreateTab() {
        JPanel tab = new JPanel(new BorderLayout(10, 10));
        tab.setBackground(Color.WHITE);
        tab.add(createAppointmentCreateForm(), BorderLayout.NORTH);
        return tab;
    }

    private JPanel createAppointmentUpdateTab() {
        JPanel tab = new JPanel(new BorderLayout(10, 10));
        tab.setBackground(Color.WHITE);
        tab.add(createAppointmentTablePanel(), BorderLayout.CENTER);
        tab.add(createAppointmentUpdateForm(), BorderLayout.SOUTH);
        return tab;
    }
    ...
    private JPanel createAppointmentCreateForm() {
        JPanel form = new JPanel(new java.awt.GridLayout(7, 2, 8, 8));
        form.setBackground(Color.WHITE);
        createAppointmentPatientBox = new JComboBox<PatientChoice>();
        createAppointmentDoctorBox = new JComboBox<DoctorChoice>();
        createAppointmentDateField = new JTextField(LocalDate.now().toString(), 10);
        createAppointmentTimeField = new JTextField("09:00", 8);
        createAppointmentActiveBox = new JCheckBox("Active / booked", true);
        createAppointmentActiveBox.setBackground(Color.WHITE);
        createAppointmentReasonField = new JTextField(20);
        form.add(new JLabel("Patient:"));
        form.add(createAppointmentPatientBox);
        form.add(new JLabel("Doctor:"));
        form.add(createAppointmentDoctorBox);
        form.add(new JLabel("Date:"));
        form.add(createAppointmentDateField);
        form.add(new JLabel("Time:"));
        form.add(createAppointmentTimeField);
        form.add(new JLabel("Status:"));
        form.add(createAppointmentActiveBox);
        form.add(new JLabel("Reason:"));
        form.add(createAppointmentReasonField);
        JPanel actions = new JPanel(new FlowLayout(FlowLayout.LEFT, 8, 0));
        actions.setBackground(Color.WHITE);
        JButton refreshLists = new JButton("Refresh Lists");
        refreshLists.addActionListener(e -> refreshAppointmentLookups());
        JButton create = new JButton("Create Appointment");
        create.addActionListener(e -> createAppointment());
        actions.add(refreshLists);
        actions.add(create);
        form.add(new JLabel());
        form.add(actions);
        return form;
    ...
    private JPanel createAppointmentUpdateForm() {
        JPanel form = new JPanel(new java.awt.GridLayout(8, 2, 8, 8));
        form.setBackground(Color.WHITE);
        appointmentIdField = new JTextField(8);
        appointmentIdField.setEditable(false);
        appointmentIdField.setFocusable(false);
    ...
    private Appointment appointmentFromCreateFields() {
        return new Appointment(0,
                selectedPatientId(),
                selectedDoctorId(),
                parseDate(createAppointmentDateField.getText()),
                parseTime(createAppointmentTimeField.getText()),
                createAppointmentStatus(),
                createAppointmentReasonField.getText());
    }
    ...
    private void createAppointment() {
        try {
            validateCreateAppointmentFields();
            Appointment appointment = controller.createAppointment(appointmentFromCreateFields());
    ...
    private void refreshAppointmentLookups() {
        loadPatientChoices();
        loadDoctorChoices();
    }

    private void loadPatientChoices() {
        if (createAppointmentPatientBox == null) {
            return;
        }
        createAppointmentPatientBox.removeAllItems();
        List<Patient> patients = controller.viewPatients();
        if (patients != null) {
            for (Patient patient : patients) {
                if (patient != null && patient.getPatientId() > 0) {
                    createAppointmentPatientBox.addItem(new PatientChoice(patient));
                }
            }
        }
        if (createAppointmentPatientBox.getItemCount() == 0) {
            createAppointmentPatientBox.addItem(new PatientChoice(0, "No patients available"));
        }
    }

    private void loadDoctorChoices() {
        if (createAppointmentDoctorBox == null) {
            return;
        }
        createAppointmentDoctorBox.removeAllItems();
        List<Doctor> doctors = controller.viewDoctors();
        if (doctors != null) {
            for (Doctor doctor : doctors) {
                if (doctor != null && doctor.getDoctorId() > 0) {
                    createAppointmentDoctorBox.addItem(new DoctorChoice(doctor));
                }
            }
        }
        if (createAppointmentDoctorBox.getItemCount() == 0) {
            createAppointmentDoctorBox.addItem(new DoctorChoice(0, "No doctors available"));
        }
    }
    ...
    private void validateCreateAppointmentFields() {
        selectedPatientId();
        selectedDoctorId();
        parseRequiredDate(createAppointmentDateField.getText(), "Appointment date");
        parseRequiredTime(createAppointmentTimeField.getText(), "Appointment time");
        requireText(createAppointmentReasonField.getText(), "Reason");
    }

    private int selectedPatientId() {
        Object selected = createAppointmentPatientBox == null ? null : createAppointmentPatientBox.getSelectedItem();
        if (selected instanceof PatientChoice) {
            int patientId = ((PatientChoice) selected).getPatientId();
            if (patientId > 0) {
                return patientId;
            }
        }
    ...
    private int selectedDoctorId() {
        Object selected = createAppointmentDoctorBox == null ? null : createAppointmentDoctorBox.getSelectedItem();
        if (selected instanceof DoctorChoice) {
            int doctorId = ((DoctorChoice) selected).getDoctorId();
            if (doctorId > 0) {
                return doctorId;
            }
        }
    ...
    private String createAppointmentStatus() {
        return createAppointmentActiveBox.isSelected() ? "BOOKED" : "CANCELLED";
    }

    private void clearCreateAppointmentForm() {
        if (createAppointmentPatientBox != null && createAppointmentPatientBox.getItemCount() > 0) {
            createAppointmentPatientBox.setSelectedIndex(0);
        }
        if (createAppointmentDoctorBox != null && createAppointmentDoctorBox.getItemCount() > 0) {
            createAppointmentDoctorBox.setSelectedIndex(0);
        }
        createAppointmentDateField.setText(LocalDate.now().toString());
        createAppointmentTimeField.setText("09:00");
        createAppointmentActiveBox.setSelected(true);
        createAppointmentReasonField.setText("");
    }
    ...
    private static class PatientChoice {
        private final int patientId;
        private final String label;

        PatientChoice(Patient patient) {
            this(patient.getPatientId(), buildLabel(patient));
        }

        PatientChoice(int patientId, String label) {
        ...
        private static String buildLabel(Patient patient) {
            String name = clean(patient.getFullName());
            return name.length() == 0 ? "Role ID " + patient.getPatientId()
                    : "Role ID " + patient.getPatientId() + " - " + name;
        }
    }

    private static class DoctorChoice {
        private final int doctorId;
        private final String label;

        DoctorChoice(Doctor doctor) {
            this(doctor.getDoctorId(), buildLabel(doctor));
        }

        DoctorChoice(int doctorId, String label) {
        ...
        private static String buildLabel(Doctor doctor) {
            String name = clean(doctor.getName());
            String specialization = clean(doctor.getSpecialization());
            String label = name.length() == 0 ? "Role ID " + doctor.getDoctorId()
                    : "Role ID " + doctor.getDoctorId() + " - " + name;
            return specialization.length() == 0 ? label : label + " (" + specialization + ")";
        }
    }
