...
public class PatientFrame extends javax.swing.JFrame {
    ...
    private JComboBox<DoctorChoice> doctorComboBox;
        ...
        if (doctors != null) {
            for (Doctor doctor : doctors) {
                if (doctor != null && doctor.getDoctorId() > 0) {
                    doctorComboBox.addItem(new DoctorChoice(doctor));
                }
            }
        }
        if (doctorComboBox.getItemCount() == 0) {
            doctorComboBox.addItem(new DoctorChoice(0, "No doctors available"));
        }
    }
    ...
    private void fillBookingFromSelectedSchedule() {
        ...
        int modelRow = scheduleTable.convertRowIndexToModel(row);
        DefaultTableModel model = (DefaultTableModel) scheduleTable.getModel();
        selectDoctorById(parseRequiredInt(text(model.getValueAt(modelRow, 2)), "Doctor ID"));
    ...
    private int selectedDoctorId() {
        Object selected = doctorComboBox == null ? null : doctorComboBox.getSelectedItem();
    ...
    private void selectDoctorById(int doctorId) {
        if (doctorComboBox == null || doctorId <= 0) {
            return;
        }
        for (int index = 0; index < doctorComboBox.getItemCount(); index++) {
            DoctorChoice choice = doctorComboBox.getItemAt(index);
            if (choice != null && choice.getDoctorId() == doctorId) {
                doctorComboBox.setSelectedIndex(index);
                return;
            }
        }
        doctorComboBox.addItem(new DoctorChoice(doctorId, "Role ID " + doctorId));
    ...
    private static class DoctorChoice {
        ...
        private static String buildLabel(Doctor doctor) {
            ...
            String label = "Role ID " + doctor.getDoctorId();
            if (name.length() > 0) {
                label += " - " + name;
            }
