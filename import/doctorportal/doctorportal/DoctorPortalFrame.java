...
public class DoctorPortalFrame extends JFrame {
    ...
    private void connectToRmiServer() {
        try {
            ...
            // Use sample data if RMI is not available
            //addSampleData();
        }
    }
...
//                }
    ...
    private void updateAppointmentLists() {
        ...
        for (DoctorAppointment apt : allAppointments) {
            ...
            if(apt.getDiagnosis() != null&& !apt.getDiagnosis().isEmpty()){
    ...
    private JPanel createPatientListPanel() {
        ...
        panel.setBorder(BorderFactory.createTitledBorder(
            ...
            "Patients with Consultation History",
            TitledBorder.LEFT, TitledBorder.TOP,
            new Font("Segoe UI", Font.BOLD, 14)));

        String[] columns = {"Patient ID", "Patient Name", "Total Consultations"};
        DefaultTableModel patientModel = new DefaultTableModel(columns, 0) {
            @Override
            public boolean isCellEditable(int row, int column) { return false; }
        };
        ...
        for (DoctorAppointment apt : allAppointments) {
            if (apt.getConsultations() != null && !apt.getConsultations().isEmpty()) {
                ...
                if (!patientMap.containsKey(patientId)) {
                    ...
                    consultationCount.put(patientId, 0);
                }
                consultationCount.put(patientId, consultationCount.get(patientId) + apt.getConsultations().size());
            }
        }
        ...
        for (Map.Entry<Integer, String> entry : patientMap.entrySet()) {
            patientModel.addRow(new Object[]{
                entry.getKey(),
                entry.getValue(),
                consultationCount.get(entry.getKey())
            });
        }
        ...
        // Store reference to patient table for selection listener
        panel.putClientProperty("patientTable", patientTable);
        panel.putClientProperty("patientModel", patientModel);
        ...
        patientTable.getSelectionModel().addListSelectionListener(e -> {
            if (!e.getValueIsAdjusting()) {
                ...
                if (row != -1) {
                    ...
                    updateConsultationTable(patientId);
                }
            }
        });
            ...
            appointment.addConsultation(newConsultation);
                ...
                if (connectedToRMI && doctorService != null) {
                    ...
                    if (saved) {
                        ...
                        // DO NOT modify appointment status
                        // Just refresh the tables
